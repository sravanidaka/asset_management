  ...
  const fetchFinancialEvents = async () => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        message.error("Failed to load financial events: " + (error.message || "Unknown error"));
      }
      ...
      render: (_, record) => (
        <Space>
          ...
          {/* <Popconfirm
            ...
            cancelText="No"
          >
            <Button type="text" icon={<FaTrash />} danger title="Delete" />
          </Popconfirm> */}
        </Space>
      ),
    },
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <div className="d-flex gap-2">
          <ExportButton
            data={dataSource}
            columns={columns}
            filename="Financial_Events_Report"
            title="Financial Events Report"
            reportType="financial-events"
